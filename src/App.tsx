      ...
      >
        ...
        {!isSharedView && (
          <header className="glass-header sticky top-0 z-30 flex shrink-0 items-center gap-2 px-3 md:px-5 transition-colors duration-300" style={{ paddingTop: 'max(env(safe-area-inset-top), 10px)', paddingBottom: '0.5rem', minHeight: 'calc(3.25rem + max(env(safe-area-inset-top), 10px))' }}>
            {(currentPage === "settings" || !currentPage.startsWith("settings")) && <SidebarTrigger className="-ml-1 shrink-0" />}
            {currentPage.startsWith("settings-") && currentPage !== "settings" && (
              <Button type="button" variant="ghost" size="sm" className="mr-0.5 flex items-center gap-2 text-foreground hover:text-primary" onClick={() => handlePageChange("settings") }>
